...
| `main` | `ml_veto.load_model_and_meta` / `predict_ok` | Возвращает `(model, meta)` с валидацией weekly precision; `predict_ok` → `PredictOutcome` (tuple-подобный: `ok`, `proba`, `threshold`, `factor`, `band`, …) и подгружает свечи/снапшоты. | OK, но при деградации модель возвращается `None`, поэтому требуется корректная мета. 【F:main.py†L2268-L2331】【F:ml_veto.py†L325-L458】 |
